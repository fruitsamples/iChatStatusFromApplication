...
@property (copy, readonly) NSNumber *objectClass;  // The class of the object.
...
- (void) loginFinishedFor:(iChatService *)for_;  // This handler is called when iChat finishes logging in.
- (void) logoutFinishedFor:(iChatService *)for_;  // This handler is called when iChat finished logging  out.
- (void) chatRoomMessageReceived:(NSString *)x from:(iChatBuddy *)from for:(iChatTextChat *)for_;  // This handler is called for incoming messages in chat rooms. If the handler returns a text value, it will replace the incoming message.
- (void) messageReceived:(NSString *)x from:(iChatBuddy *)from for:(iChatTextChat *)for_;  // This handler is called for incoming messages. If the handler returns a text value, it will replace the incoming message.
- (void) messageSent:(NSString *)x for:(iChatTextChat *)for_;  // This handler is called for outgoing messages. If the handler returns a text value, it will replace the outgoing message.
- (void) receivedTextInvitation:(NSString *)x from:(iChatBuddy *)from for:(iChatTextChat *)for_;  // This handler is called for incoming text invitations.
- (void) receivedAudioInvitationFrom:(iChatBuddy *)from for:(iChatAudioChat *)for_;  // This handler is called for incoming audio invitations.
- (void) receivedVideoInvitationFrom:(iChatBuddy *)from for:(iChatVideoChat *)for_;  // This handler is called for incoming video invitations.
...
@property (readonly) int id;  // The unique identifier of the window.
@property int index;  // The index of the window, ordered front to back.
...
@property (copy) NSColor *color;  // The color of the first character.
@property (copy) NSString *font;  // The name of the font of the first character.
@property int size;  // The size in points of the first character.
...
@property (readonly) int idleTime;  // Time in seconds that I have been idle.
@property NSData *image;  // My image as it appears in all services.
...
@property (copy, readonly) NSString *id;  // The buddy's service and handle. For example: AIM:JohnDoe007
...
@property (readonly) int idleTime;  // The time in seconds the buddy has been idle.
@property (copy, readonly) NSArray *capabilities;  // The buddy's messaging capabilities.
@property (readonly) NSData *image;  // The buddy's custom image.
...
- (void) buddyBecameAvailable;  // This hander is called when a buddy becomes available
- (void) buddyBecameUnavailable;  // This hander is called when a buddy becomes unavailable
...
@property (copy, readonly) NSString *id;  // A guid identifier for this service.
...
@property (copy, readonly) NSString *id;  // A guid identifier for this chat.
...
- (void) avChatStarted;  // This handler is called when a buddy accepts your audio or video invitation.
- (void) avChatEnded;  // This handler is called when an audio or video chat ends.
...
@property (copy, readonly) NSString *id;  // The guid for this file transfer
...
@property (readonly) int fileSize;  // The total size in bytes of the completed file transfer
@property (readonly) int fileProgress;  // The number of bytes that have been transferred
...
- (void) accept;  // Accepts an incoming text, audio, or video chat invitation, or file transfer
- (void) decline;  // Declines an incoming text, audio, or video chat invitation, or file transfer
- (void) receivedFileTransferInvitation;  // This handler is called when a buddy sends you a file.
- (void) completedFileTransfer;  // This handler is called when a file transfer is completed.
